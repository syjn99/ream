...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("cargo:rustc-env=REAM_BUILD_OPERATING_SYSTEM={build_operating_system}");

    // The architecture of the build, x86_64, aarch64, etc.
    println!("cargo:rustc-env=REAM_BUILD_ARCHITECTURE={build_architecture}");

    println!(
        "cargo:rustc-env=REAM_FULL_VERSION={}",
        format_args!(
            "{version}-{hash} {build_os}-{build_arch} rustc{rust_version}",
            version = ream_version,
            hash = short_sha,
