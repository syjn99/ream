...
fn preprocess_help(s: &str) -> Cow<'_, str> {
    static REPLACEMENTS: LazyLock<Vec<(Regex, &str)>> = LazyLock::new(|| {
        let patterns: &[(&str, &str)] = &[
            ...
            (r"default: /.*/ream", "default: <CACHE_DIR>"),
            ...
            (
                r"default: ream/.*-[0-9A-Fa-f]{6,10}/([_\w]+)-(\w+)-(\w+)(-\w+)?",
                "default: ream/<VERSION>-<SHA>/<ARCH>",
            ),
            // Remove the OS
            (r"default: ream/.*/\w+", "default: ream/<VERSION>/<OS>"),
...
struct Cmd<'a> {
    /// path to binary (e.g. ./target/debug/ream)
    cmd: &'a Path,
